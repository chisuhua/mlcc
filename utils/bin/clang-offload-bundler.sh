#!/bin/bash
#
#  $MLCC/bin/clang-offload-bundler.sh
#
...
if [ "$inputfiletype" == " current ar archive" ] ; then
   ...
   mkdir -p $tmpdir
   cd $tmpdir
   $MLCC/bin/llvm-ar x $curdir/$inputfile
   hostfiles=""
   devicefiles=""
   for f in `$MLCC/bin/llvm-ar t $curdir/$inputfile` ; do
      $MLCC/bin/clang-offload-bundler $1 $2 -inputs=$f -outputs=host.$f,$f.bc $5
      rm $f
   ...
   if [ "$devicefiletype" == "ELF" ] ; then
     ...
     $MLCC/bin/llvm-link $devicefiles -o $devicefile
   fi
   rm -rf $tmpdir
else
  # not an archive, use the original clang-offload-bundler
  $MLCC/bin/clang-offload-bundler $@
fi
